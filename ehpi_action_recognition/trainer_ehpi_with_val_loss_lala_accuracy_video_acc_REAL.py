...
class TrainerEhpi(object):
        ...
        for epoch in range(train_config.num_epochs):
            ...
            with torch.no_grad():
                ...
                for i,data in enumerate(test_loader):
                    ...
                    batch = 128
                    outputs,hidden = model(input_LSTM, h_2,batch,64)
            ...
            if test_loader is not None:
                accuracy,accuracy_video,accuracy_vid_real = self.test(model, test_loader=test_loader)
                ...
                tb.add_scalar('Accuracy_Test_VIDEO_REAL',accuracy_vid_real,epoch)
                tb.add_scalars('Accuracy_Together',{'Accuracy_Clip' :accuracy, 'Accuracy_Video_REAl':accuracy_vid_real},epoch)   
    ...
    def test(self, model, test_loader: DataLoader):
        ...
        for i, data in enumerate(test_loader):
            ...
            outputs,hidden = model(input_LSTM, h_3,batch,32)
            ...
            for i in range (127):        
                if i <= 121 : 
                    if i == 0 and (seq[i] == seq[i+6]):
                        outputs_sequence_total = outputs[i] + outputs[i+1] + outputs[i+2] + outputs[i+3] + outputs[i+4] + outputs[i+5] + outputs[i+6] 
                        outputs_div = outputs_sequence_total/7
                        predictions_vid = np.argmax(outputs_div)
                        
                        correct_vid = predictions_vid == lala[i]
                        corrects_vid.append(int(correct_vid))
                        ...
                        outputs_sequence_total = outputs[i] + outputs[i+1] + outputs[i+2] + outputs[i+3] + outputs[i+4] + outputs[i+5] 
                        outputs_div = outputs_sequence_total/6
                        predictions_vid = np.argmax(outputs_div)
                        ...
                        outputs_sequence_total = outputs[i] + outputs[i+1] + outputs[i+2] + outputs[i+3] + outputs[i+4]
                        outputs_div = outputs_sequence_total/5
                        predictions_vid = np.argmax(outputs_div)
                        ...
                        outputs_sequence_total = outputs[i] + outputs[i+1] + outputs[i+2] + outputs[i+3] 
                        outputs_div = outputs_sequence_total/4
                        predictions_vid = np.argmax(outputs_div)
                        ...
                        outputs_sequence_total = outputs[i] + outputs[i+1] + outputs[i+2]
                        outputs_div = outputs_sequence_total/3
                        predictions_vid = np.argmax(outputs_div)
                        ...
                        outputs_sequence_total = outputs[i] + outputs[i+1]
                        outputs_div = outputs_sequence_total/2
                        predictions_vid = np.argmax(outputs_div)
                        ...
                        outputs_sequence_total = outputs[i]
                        outputs_div = outputs_sequence_total/1
                        predictions_vid = np.argmax(outputs_div)
                        ...
                        prediction_sequence =  np.argmax(outputs)
                        true_false = np.equal(prediction_sequence, lala[i])
                        true = np.sum(true_false)
                        correct_rate =  (true*100)/1
                        corrects_total_video.append(correct_rate)                        
        ...
        accuracy_vid_real = float(sum(corrects_vid)) / float(len(corrects_vid))
        ...
        print("accuracy video REAL:")
        print(accuracy_vid_real)
        ...
        return accuracy,accuracy_video,accuracy_vid_real
