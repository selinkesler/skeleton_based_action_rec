        ...
        for i, data in enumerate(test_loader):
            ...
            for i in range (121):        
                ...
                if i == 0 and (seq[i] == seq[i+6]):
                    prediction_sequence =  np.argmax(outputs[i:i+7])
                    correct_sequnce = prediction_sequence == y[i:i+7]
                    corrects_sequence.append(int(correct_sequnce))
                elif i == 0 and (seq[i] == seq[i+5]):
                    prediction_sequence =  np.argmax(outputs[i:i+6])
                    correct_sequnce = prediction_sequence == y[i:i+6]
                    corrects_sequence.append(int(correct_sequnce))              
                elif i == 0 and (seq[i] == seq[i+4]):
                    prediction_sequence =  np.argmax(outputs[i:i+5])
                    correct_sequnce = prediction_sequence == y[i:i+5]
                    corrects_sequence.append(int(correct_sequnce))              
                elif i == 0 and (seq[i] == seq[i+3]):
                    prediction_sequence =  np.argmax(outputs[i:i+4])
                    correct_sequnce = prediction_sequence == y[i:i+4]
                    corrects_sequence.append(int(correct_sequnce))              
                elif i == 0 and (seq[i] == seq[i+2]):
                    prediction_sequence =  np.argmax(outputs[i:i+3])
                    correct_sequnce = prediction_sequence == y[i:i+3]
                    corrects_sequence.append(int(correct_sequnce))              
                elif i == 0 and (seq[i] == seq[i+1]):
                    prediction_sequence =  np.argmax(outputs[i:i+2])
                    correct_sequnce = prediction_sequence == y[i:i+2]
                    corrects_sequence.append(int(correct_sequnce))  
