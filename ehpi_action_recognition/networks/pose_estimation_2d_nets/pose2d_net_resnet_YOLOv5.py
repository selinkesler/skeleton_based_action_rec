...
from ehpi_action_recognition.networks.detection_nets.detection_net_yolo_v5 import DetectionNetYoloV3, \
...
class Pose2DNetResnet(Pose2DNetBase):
    ...
    def __init__(self, model: PoseResNet, skeleton_type: Type[SkeletonBase]):
        ...
        self.model = model
        self.detector = DetectionNetYoloV3()

    def get_humans_from_img(self, img: np.ndarray) -> List[Human]:
        bbs = self.detector.detect(img)
        humans: List[Human] = []
        for bb in bbs:
