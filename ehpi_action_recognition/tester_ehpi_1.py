...
class TesterEhpi(object):
    def test(self, test_loader: DataLoader, weights_path: str, model):
        ...
        model.to(device)
        ehpi_results ,ehpi_accuracy = self.test_ehpis(model, test_loader)
        sequence_results ,sequence_accuracy = self.test_sequences(model, test_loader)
        return ehpi_results, ehpi_accuracy, sequence_results , sequence_accuracy
