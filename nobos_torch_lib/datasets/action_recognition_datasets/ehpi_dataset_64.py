...
class EhpiDataset_64(Dataset):
    ...
    def load_X(self):
        ...
        X_ = np.reshape(X_, (X_.shape[0], 64, self.num_joints, 3))
       ...
       # print(index)
        ...
        if self.transform:
            try:
                ...
                pass
             #   raise err
      #  print("sonraki sample hali : ")
      #  print(sample)
        
      #  print(float(sample['x']))
    ...
    def __getitem__(self, index):
        ...
        # print("int frame_len :")
        # print(frame_len) 
        a = np.reshape(a, (frame_len, self.num_joints, 3))
        a = np.transpose(a,(2, 0, 1))
        a[2, :, :] = 0
        # print(a)
    
        
        sample = {"x": a.copy(), "y": self.y[index][0], "seq": self.y[index][1]}
        # sample = {"x": self.x[index].copy(), "y": self.y[index][0], "seq": self.y[index][1]}
...
class RemoveJointsOutsideImgEhpi_64(object):
    ...
    def __call__(self, sample):
      ...
      #  print(self.image_size.height)
        ...
        sample['x'] = ehpi_img
       # print("RemoveJointsOutsideImgEhpi done")
        return sample

class ScaleEhpi_64(object):
    def __init__(self, image_size: ImageSize):
        # if image_size.width > image_size.height:
        #     self.image_size = ImageSize(width=1, height=image_size.height/image_size.width)
        # else:
    ...
    def __call__(self, sample):
        ...
        ehpi_img[1, :, :][tmp[1, :, :] == 0] = 0
        sample['x'] = ehpi_img
       # print("ScaleEhpi done")
        return sample


class TranslateEhpi_64(object):
    def __init__(self, image_size: ImageSize):
        self.image_size = image_size
        # if image_size.width > image_size.height:
        #     self.image_size = ImageSize(width=1, height=image_size.height/image_size.width)
    ...
    def __call__(self, sample):
        ...
        ehpi_img[1, :, :][tmp[1, :, :] == 0] = 0
        sample['x'] = ehpi_img
      #  print("TranslateEhpi done")
        return sample


class NormalizeEhpi_64(object):
    def __init__(self, image_size: ImageSize):
        if image_size.width > image_size.height:
    ...
    def __call__(self, sample):
       ...
       # max_factor_y = 1 / np.max(ehpi_img[1, :, :])
        ...
        sample['x'] = ehpi_img
       # print("NormalizeEhpi done")
        return sample
        
class GaussianNoise_64(object):
    def __init__(self, image_size: ImageSize):
        # if image_size.width > image_size.height:
        #     self.image_size = ImageSize(width=1, height=image_size.height/image_size.width)
        # else:
    ...
    def __call__(self, sample):
        ...
        noise_1 = np.random.normal(mu, sigma, [64,15])
        noise_2 = np.random.normal(mu, sigma, [64,15])      
    
        
        '''
        noise_1 = np.random.normal(mu, sigma, [1,480])
        noise_2 = np.random.normal(mu, sigma, [1,480])
     
        # adding noise to ehpi_img
        # should I add same noise or different noises to x and y coordinates?
        
        new_array_x = np.zeros((32,15))
        new_array_y = np.zeros((32,15))
        
        new_array_x = ehpi_img[0, :, :]
        new_array_y = ehpi_img[1, :, :]
        
        new_array_x = new_array_x.reshape(1,480)
        new_array_y = new_array_y.reshape(1,480)
        
        #print("x before noise:")
        #print(ehpi_img[0, :, :])
        ...
        for i in range(480):
            new_array_x[0,i] = new_array_x[0,i] + noise_1[0,i]
            new_array_y[0,i] = new_array_y[0,i] + noise_2[0,i]
            
        new_array_x = new_array_x.reshape(32,15)
        new_array_y = new_array_y.reshape(32,15)
        
        #print(new_array_x.shape)
        #print(ehpi_img.shape)
        #print(ehpi_img[0, :, :].shape)
        
        ehpi_img[0, :, :] = new_array_x
        ehpi_img[1, :, :] = new_array_y
        
        #print("x after noise:")
        #print(ehpi_img[0, :, :])
        '''
        
        ehpi_img[0, :, :] = ehpi_img[0, :, :] + noise_1
        ehpi_img[1, :, :] = ehpi_img[1, :, :] + noise_2
        # print(ehpi_img[0, :, :])      
       
        tmp = np.copy(ehpi_img)
       
        # if after adding noise, new coordinate > 1 -- should be 1
        # if after adding noise, new coordinate < 0 -- should be 0
        ehpi_img[0, :, :][tmp[0, :, :] > 1] = 1
        ehpi_img[0, :, :][tmp[0, :, :] < 0] = 0
       
        ehpi_img[1, :, :][tmp[0, :, :] > 1] = 1
        ehpi_img[1, :, :][tmp[0, :, :] < 0] = 0
...
class GaussianNoise_01_64(object):
    def __init__(self, image_size: ImageSize):
        # if image_size.width > image_size.height:
        #     self.image_size = ImageSize(width=1, height=image_size.height/image_size.width)
        # else:
...
class RemoveJointsEhpi_64(object):
